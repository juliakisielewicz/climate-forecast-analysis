...
for (i in 1:tmax_fore_dim[1]){
	for (j in 1:tmax_fore_dim[2]){
		print(i)
		a<-as.vector(spei( as.numeric(ppt_fore_array[i,j,]) - as.numeric(PET_fore[i,j,]), 6)) 
...
for (i in july_fore)
{
  ...
  spei_fore_r_cropped <- crop(spei_fore_raster, ex)
  spei_fore_r_masked <- mask(spei_fore_r_cropped, as_Spatial(both))

  plot(spei_fore_r_masked, main=paste("6-month SPEI based on forecasted data - ", time_real[i]), col=pal(12), breaks=cuts)
