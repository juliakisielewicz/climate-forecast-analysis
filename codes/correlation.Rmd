...
```{r}
load("manual_real_fore_rasters.RData")
...
for (i in seq(1,12)) {
  ...
  #spearman
  corr_s <- corLocal(tmax_rasters$real[i][[1]] , tmax_rasters$forecast[i][[1]], method="spearman", test=TRUE)
...
for (i in seq(1,12)) {
  ...
  #spearman
  corr_s <- corLocal(tmin_rasters$real[i][[1]] , tmin_rasters$forecast[i][[1]], method="spearman", test=TRUE)
