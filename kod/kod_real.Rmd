...
length(lon_real == lon_real_tmin) == length(lon_real_tmin) #TRUE
length(lat_real == lat_real_tmin) == length(lat_real_tmin) #TRUE
length(time_real == time_real_tmin) == length(time_real_tmin) #TRUE
...
Visualization
```{r} 
range(na.omit(as.numeric(tmin_real_array)))
range(na.omit(as.numeric(lon_real)))
range(na.omit(as.numeric(lat_real)))
...
#checking if new variables are necessary
length(lon_real == lon_real_ppt) == length(lon_real_ppt) #TRUE
length(lat_real == lat_real_ppt) == length(lat_real_ppt) #TRUE
length(time_real == time_real_ppt) == length(time_real_ppt) #TRUE
#deleting unnecessary variables
...
Visualization
```{r} 
range(na.omit(as.numeric(ppt_real_array)))
range(na.omit(as.numeric(lon_real)))
range(na.omit(as.numeric(lat_real)))
...
lon<-lon_real
lat<-lat_real
levelplot(ppt_real_array[,,7] ~ lon * lat, data=grid_real, at=cutpts, cuts=11, pretty=T, col.regions=(rev(brewer.pal(10,"RdBu"))),aspect=range_real_lat/range_real_lon)
...
tmp_pet <- hargreaves(tmin_real_array, tmax_real_array, lat=lat_real)
```
