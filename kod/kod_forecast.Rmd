...
RUNNING NECCESSARY FILES
```{r}
source_rmd("D:/AGH/INZ/kod/kod_real.Rmd")
print("running kod_real - success")
```
LOADING SAVED WORKSPACE    
```{r}
load("D:/AGH/INZ/kod/manual_save.RData")
source_rmd("D:/AGH/INZ/kod/kod_shapefiles.Rmd")
...
levelplot(z ~ lon * lat, data=grid_fore, at=cuts, pretty=T, col.regions=(rev(brewer.pal(12,"RdBu"))),aspect=range_fore_lat/range_fore_lon, main = "Forecasted maximum temperature - July 2006")
...
for (i in 1 : tmax_fore_dim[1])
{
    ...
    {
        print(i)
        PET_fore[i,j,] <- hargreaves(tmin_fore_array[i,j,], tmax_fore_array[i,j,], lat=lat_fore[j])
    }
}
...
SPEI
```{r}
SPEI_fore <- array(rep(NA, prod(tmax_fore_dim)), dim = tmax_fore_dim)

for (i in 1:tmax_fore_dim[1]){
	for (j in 1:tmax_fore_dim[2]){
		print(i)
		a<-as.vector(spei( as.numeric(ppt_fore_array[i,j,]) - as.numeric(PET_fore[i,j,]), 6)) # ostatni parametr -> liczba miesi�cy
		SPEI_fore[i,j,]<-as.vector(a$fitted)		
	}
}

range(na.omit(as.numeric(SPEI_fore[,,175])))
...
for (i in 7:7)
{
  ...
  plot(spei_fore_raster,main=paste("6-month SPEI based on forecasted data - ", time_real[i]),col=pal(12),breaks=cuts)
  plot(both,add=TRUE,lwd=2)
  
  ## crop and mask
  ex <- extent(as_Spatial(both))
  r2 <- crop(spei_fore_raster, ex)
  r3 <- mask(r2, as_Spatial(both))

  plot(r3,main=paste("6-month SPEI based on forecasted data - ", time_real[i]),col=pal(12),breaks=cuts)
  plot(both, add=TRUE, lwd=2)
}
```
